...
describe('regex functions', () => {
  ...
  describe('REGEXP_LIKE', () => {
    it('should return true when string matches pattern', async () => {
      ...
      const result = await collect(executeSql({
        tables: { data },
        query: 'SELECT REGEXP_LIKE(text, \'[0-9]+\') AS has_num FROM data',
      }))
      expect(result).toEqual([
        { has_num: true },
        { has_num: false },
    ...
    it('should support anchored patterns', async () => {
      ...
      const result = await collect(executeSql({
        tables: { data },
        query: 'SELECT REGEXP_LIKE(code, \'^ABC\') AS m FROM data',
      }))
      expect(result).toEqual([
        { m: true },
        { m: false },
    ...
    it('should handle null string value', async () => {
      ...
      const result = await collect(executeSql({
        tables: { data },
        query: 'SELECT REGEXP_LIKE(text, \'[a-z]+\') AS m FROM data',
      }))
      expect(result[0].m).toBeNull()
    })
    ...
    it('should handle null pattern value', async () => {
      ...
      const result = await collect(executeSql({
        tables: { data },
        query: 'SELECT REGEXP_LIKE(text, pattern) AS m FROM data',
      }))
      expect(result[0].m).toBeNull()
    })
    ...
    it('should throw for invalid regex pattern', async () => {
      ...
      await expect(collect(executeSql({
        tables: { data },
        query: 'SELECT REGEXP_LIKE(text, \'[\') FROM data',
      }))).rejects.toThrow('invalid regex pattern: Invalid regular expression: /[/: Unterminated character class')
    })
    ...
    it('should throw for wrong argument count', () => {
      ...
      expect(() => executeSql({
        tables: { data },
        query: 'SELECT REGEXP_LIKE(text) FROM data',
      })).toThrow('REGEXP_LIKE(string, pattern) function requires 2 arguments, got 1')
    })
    ...
    it('should work in WHERE clause', async () => {
      ...
      const result = await collect(executeSql({
        tables: { data },
        query: 'SELECT id FROM data WHERE REGEXP_LIKE(text, \'[0-9]+\')',
      }))
      expect(result).toEqual([{ id: 1 }, { id: 3 }])
    })
  })
