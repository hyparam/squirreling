...
describe('executeSql - GROUP BY', () => {
  ...
  it('should group by single column', () => {
    const result = executeSql({ source, sql: 'SELECT city, COUNT(*) AS count FROM users GROUP BY city' })
  ...
  it('should group by multiple columns', () => {
    const result = executeSql({ source, sql: 'SELECT city, age, COUNT(*) AS count FROM users GROUP BY city, age' })
  ...
  it('should handle aggregates with GROUP BY', () => {
    const result = executeSql({ source, sql: 'SELECT city, AVG(age) AS avg_age FROM users GROUP BY city' })
    expect(result).toHaveLength(2)
    const nycGroup = result.find(r => r.city === 'NYC')
  ...
  it('should select non-grouped column (takes first value)', () => {
    const result = executeSql({ source, sql: 'SELECT city, name, COUNT(*) AS count FROM users GROUP BY city' })
    expect(result).toHaveLength(2)
  ...
  it('should handle empty groups with SELECT *', () => {
    ...
    const result = executeSql({ source: data, sql: 'SELECT * FROM users WHERE age > 100 GROUP BY city' })
  ...
  it('should aggregate with multiple group columns', () => {
    ...
    const result = executeSql({ source: sales, sql: `
