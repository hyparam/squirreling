...
export function executeTopN(plan, context) {
  ...
  return {
    ...
    async *rows() {
      ...
      // Heap operations (0-indexed binary heap)
      function siftDown(i) {
        const n = heap.length
        while (true) {
          let worst = i
          const left = 2 * i + 1
          const right = 2 * i + 2
          // "worst" = the entry that should be at the top (the one we'd evict)
          // For the heap, parent > children means parent is the eviction candidate
