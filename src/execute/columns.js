...
function collectColumnsFromExpr(expr, columns) {
  ...
  } else if (expr.type === 'between' || expr.type === 'not between') {
    collectColumnsFromExpr(expr.expr, columns)
    collectColumnsFromExpr(expr.lower, columns)
    collectColumnsFromExpr(expr.upper, columns)
  } else if (expr.type === 'in valuelist' || expr.type === 'not in valuelist') {
    collectColumnsFromExpr(expr.expr, columns)
    for (const val of expr.values) {
      collectColumnsFromExpr(val, columns)
    }
  } else if (expr.type === 'in' || expr.type === 'not in') {
    collectColumnsFromExpr(expr.expr, columns)
